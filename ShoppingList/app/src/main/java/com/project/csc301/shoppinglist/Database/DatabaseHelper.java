...
public class DatabaseHelper extends Activity {
    ...
    public int addNewSetting(int setting, int val) {
        int get = getSettingVal(setting);
        if (getSettingVal(setting) >= 0)
            return get;
        ContentValues cv = new ContentValues();
        cv.put(DBHelper.SETTINGS_ID, setting);
        cv.put(DBHelper.SETTINGS_VALUE, val);
        SQLiteDatabase db = DBHelper.getWritableDatabase();
        long pid = db.insert(DBHelper.TABLE_SETTINGS, null, cv);
        db.close();
        return getSettingVal(setting);
    }

    public int getSettingVal(int setting) {
        String query = "SELECT " + DBHelper.SETTINGS_VALUE + " FROM " + DBHelper.TABLE_SETTINGS + " WHERE " + DBHelper.SETTINGS_ID + " =?";
        String[] arg = new String[]{String.valueOf(setting)};

        SQLiteDatabase db = DBHelper.getWritableDatabase();
        Cursor cursor = db.rawQuery(query, arg);

        if (cursor.moveToNext()) {
            int sid = cursor.getInt(cursor.getColumnIndex(DBHelper.SETTINGS_VALUE));
            db.close();
            return sid;
        } else {
            return -1;
    ...
    public int updateSetting(int setting, int val) {
        ContentValues cv = new ContentValues();
        cv.put(DBHelper.SETTINGS_VALUE, val);
        String wheresql = DBHelper.SETTINGS_ID + "=?";
        String[] args = new String[]{String.valueOf(setting)};

        SQLiteDatabase db = DBHelper.getWritableDatabase();
        int updates = db.update(DBHelper.TABLE_SETTINGS, cv, wheresql, args);
        db.close();
        return updates;
    }
