...
public class ShoppingListMain
    ...
    private MenuItem additem, delete, delete_all,
            canceldelete, delete_all_checked, show_hide_trip, toggle_menu;

    private boolean ON_DELETE_MODE, MENU_ONLY, SHOWING_STORES, ON_SIMPLE_MODE;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ON_SIMPLE_MODE = dbh.addNewSetting(0, 1) == 1 ? true : false;
        } catch (Exception e) {
            GlobalFunctions.showInformativeDialog(this, "Error", e.getMessage());
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        toggle_menu = this.menu.findItem(R.id.action_app_mode);
        toggle_menu.setChecked(ON_SIMPLE_MODE);
        refreshData();
        return true;
    }
    ...
    public void refreshData() {
        ...
        showStoresProcess(SHOWING_STORES && !ON_SIMPLE_MODE);
        if (SHOWING_STORES && !ON_SIMPLE_MODE)
    ...
    public void toggleSimpleMode(MenuItem m) {
        int value = dbh.getSettingVal(0);
        int newValue = value == 1 ? 0 : 1;
        Log.i("Settings", String.valueOf(newValue));
        newValue = dbh.updateSetting(0, newValue);
        Log.i("Settings", String.valueOf(newValue));
        ON_SIMPLE_MODE = dbh.getSettingVal(0) == 1;
        Log.i("Settings", String.valueOf(ON_SIMPLE_MODE));
        refreshData();
    }

    public void toggleSimpleMode() {
        toggle_menu.setChecked(ON_SIMPLE_MODE);
        showMenuItem(show_hide_trip, !ON_SIMPLE_MODE);
    }
    ...
    public void onLongClick(ItemModel m) {
        if (!ON_DELETE_MODE && !ON_SIMPLE_MODE) {
